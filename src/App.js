...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  function handleAdd() {
    const newWatchedMovie = {
      ...
      year,
      poster,
      imdbRating: runtime ? Number(runtime.split(" ").at(0)) : 0,
...
function MoviesWatchedSummary({ watched, onDeleteWatched }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <WatchedMovie
...
function Movie({ movie, onSelectMovie }) {
  return (
    <li onClick={() => onSelectMovie(movie.imdbID)} tabIndex={0} style={{ cursor: "pointer" }}>
      <img src={movie.Poster} alt={`${movie.Title} poster`} />
      <h3>{movie.Title}</h3>
      <MoviesYear movie={movie} />
    </li>
  );
}

function MoviesYear({ movie }) {
  return (
    <div>
      <p>
